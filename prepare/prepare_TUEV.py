...
data_root = "/nvme/public/datasets/EEG/tuh_eeg_events/v2.0.1/edf"
root = "/nvme/public/users/czl/EEG-LM-GRPO/data/TUEV"
...
BaseDirTrain = os.path.join(data_root, "train")
fs = 200
...
BaseDirEval = os.path.join(data_root, "eval")
fs = 200
...
train_sub = list(set([f.split("-")[0] for f in train_files]))
...
val_files = [f for f in train_files if f.split("-")[0] in val_sub]
train_files = [f for f in train_files if f.split("-")[0] in train_sub]
